...
{#snippet carouselData({header, isi}: carouselData,key: number)}
    <div id={String(key)} class="bg-slate-900 backdrop-blur-md hover:text-slate-900 hover:bg-white transition duration-300 border border-slate-800/60 rounded-t-3xl rounded-b-lg grid w-[100%] h-[6rem] p-4 grid-rows-[30%_5%_65%]">
    <div>
        <h2>{header}</h2>
    </div>
    <div></div>
    <ul>
        <li class="text-sm/tight">
            {isi}
        </li>
    </ul>
    </div>
...
<section id="kelebihan-app" class="w-full h-[24rem] p-4 grid grid-cols-[50%_5%_45%] mt-20">
    <div class=" text-6xl text-right font-signature text--900  flex justify-center items-center">
    Langkah Nya Cukup Mudah
    </div>
    <div></div>
    <!-- Container Utama (Harus relative agar shade absolute bisa menempel pas) -->
    <div class="relative h-[22rem] mt-20">
        
        <!-- SHADE ATAS: Gradasi putih ke transparan -->
        <div class="absolute top-0 left-0 right-0 h-5 bg-gradient-to-b from-white to-transparent pointer-events-none z-10"></div>

        <!-- WADAH SCROLL KAMU -->
        <div use:scrollEnd class="h-full overflow-y-auto p-2 scrollbar-none grid space-y-2 scroll-my-[100%] text-white transition-colors duration-300 ease-in-out">
            {#each headeramaisi as d, i}
                {@render carouselData(d, i)}
            {/each}
        </div>

        <!-- SHADE BAWAH: Gradasi transparan ke putih -->
